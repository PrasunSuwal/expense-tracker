...
def api_insights():
    # POST can send bootstrap estimates for new users
    user_estimates = {}
    if request.method == "POST":
        user_estimates = request.json or {}

    data = fetch_data()
    df = prepare_dataframe(data)

    if df.empty:
        # No historical data - fallback to user estimates for forecast & insights
        # Create dummy dataframe with monthly user estimates split weekly
        weekly_labels = []
        today = datetime.now()
        for i in range(1,5):
        ...
        # Assume user_estimates contains categories with positive (expense/revenue) values
        for cat, amount in user_estimates.items():
            # classify 'Salary' and 'Freelance' as revenue, rest as expense (simple heuristic)
            if cat.lower() in ["salary", "freelance"]:
                revenue_sum += amount
            else:
                expense_sum += amount
                # Spread monthly amount equally over 4 weeks
                forecast_results[cat] = [round(amount / 4, 2)] * 4

        # Cashflow forecast = revenue - expense weekly (equally spread)
        weekly_revenue = revenue_sum / 4
        weekly_expense = expense_sum / 4
        cashflow_values = [round(weekly_revenue - weekly_expense, 2)] * 4

        # Basic insights for new user
        insights = [
    ...
    # If we have data, proceed with full forecasting & insights
