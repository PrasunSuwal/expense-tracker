...
# ----------------------
client = MongoClient(
    "mongodb+srv://prasunsuwal:Am71NQ9zpK0FNHeZ@expensetracker.mknvcqt.mongodb.net/?retryWrites=true&w=majority&appName=expenseTracker"
)
db = client['test']
...
def forecast_monthly_expenses(df):
    ...
    ) if not monthly.empty else pd.Timestamp.now()
    months_labels = [(last_month + pd.DateOffset(months=i)
                      ).strftime('%Y-%m') for i in range(1, 4)]

    print(
        f"Debug: Monthly forecasting for {len(categories)} categories over {total_months} months")

    for cat in categories:
        cat_monthly = monthly[monthly['category']
                              == cat].set_index('date').sort_index()
        cat_weekly = weekly[weekly['category'] == cat].set_index(
            'date').sort_index() if not weekly.empty else pd.DataFrame()
        ...
        max_monthly = cat_monthly['amount'].max() if len(
            cat_monthly) > 0 else 0
        recent_monthly = cat_monthly['amount'].iloc[-2:].mean() if len(
            cat_monthly) >= 2 else avg_monthly_amount
        ...
        monthly_bills = ['rent', 'utilities', 'insurance',
                         'subscription', 'mortgage', 'loan', 'emi']
        essential_monthly = ['groceries',
                             'food'] if monthly_frequency >= 0.7 else []
        regular_monthly = ['shopping',
                           'entertainment'] if monthly_frequency >= 0.5 else []

        is_monthly_bill = cat.lower() in monthly_bills or monthly_frequency >= 0.8
        is_essential_monthly = cat.lower() in essential_monthly or (
            monthly_frequency >= 0.7 and avg_monthly_amount > 2000)
        ...
        if len(cat_monthly) >= 4:
            ...
            trend_change = (recent_trend - older_trend) / \
                older_trend if older_trend > 0 else 0
        else:
            trend_change = 0
        ...
        if is_monthly_bill:
            ...
            if monthly_frequency > 0:
                ...
                forecast_results[cat] = [
                    base_amount if i < months_to_show else base_amount * 0.2 for i in range(3)]
            else:
                forecast_results[cat] = [0, 0, 0]

        # Clean up results
        forecast_results[cat] = [round(max(0, v), 2)
...
def forecast_weekly_expenses(df):
    ...
    for cat in categories:
        ...
        print(
            f"Debug: '{cat}' - Recent avg: ₹{recent_avg:.0f}, Older avg: ₹{older_avg:.0f}, Trend: {trend_change:.1%}")
        ...
        if is_monthly_expense:
            ...
            print(
                f"Debug: '{cat}' classified as MONTHLY (₹{monthly_amount:.0f}/month, trend: {trend_change:.1%})")
            ...
            print(
                f"Debug: '{cat}' classified as MONTHLY (₹{monthly_amount:.0f}/month)")
            ...
            try:
                ...
                forecast_with_variation = [
                    ...
                    max(forecast[1] * 0.5 * trend_multiplier,
                        forecast[1] * 0.3),  # Light week
                    forecast[2] * 0.9 * trend_multiplier,
                    max(forecast[3] * 0.6 * trend_multiplier,
                        forecast[3] * 0.4)   # Another light week
                ]
                forecast_results[cat] = [
                    # Minimum ₹200
                    round(max(200, v), 2) for v in forecast_with_variation]
                print(
                    f"Debug: '{cat}' classified as REGULAR (exponential smoothing, trend: {trend_change:.1%})")
            except Exception as e:
                ...
                print(
                    f"Debug: '{cat}' classified as REGULAR (fallback, trend: {trend_change:.1%})")
        else:
            # Default case
...
def forecast_net_cashflow(df):
    ...
    df_copy = df.copy()
    df_copy['net'] = np.where(
        df_copy['type'] == 'revenue', df_copy['amount'], -df_copy['amount'])
    ...
    if len(weekly_net) < 3:
        if len(weekly_net) > 0:
            ...
            values = []
            for i in range(4):
                # Small trend continuation
                val = last_net + (trend_per_week * (i + 1) * 0.2)
        ...
        try:
            ...
            print(
                f"Debug: Cashflow exponential smoothing failed, using fallback: {e}")
            # Fallback to trend-based forecast
            recent_avg = weekly_net.iloc[-2:].mean()
            older_avg = weekly_net.iloc[:-
                                        2].mean() if len(weekly_net) > 2 else recent_avg
            trend = recent_avg - older_avg

            labels = [(weekly_net.index.max() + pd.Timedelta(weeks=i)
                       ).strftime('%Y-%m-%d') for i in range(1, 5)]
            values = [round(recent_avg + trend * (i + 1) * 0.3, 2)
                      for i in range(4)]
