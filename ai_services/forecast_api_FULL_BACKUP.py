...
def generate_synthetic_data(df, target_months=12):
    ...
    monthly_stats = df.groupby(['year_month', 'category', 'type'])['amount'].agg(['mean', 'std', 'count']).reset_index()
    ...
    for cat in df['category'].unique():
        for trans_type in df['type'].unique():
            ...
            for month_offset in range(1, months_needed + 1):
                ...
                synthetic_amount = max(synthetic_amount, avg_amount * 0.5)  # Don't go below 50% of average
...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Key lag features
    for lag in [1, 2, 3]:
        df[f'lag_{lag}'] = df.groupby('category')['amount'].shift(lag).fillna(df['amount'].mean())
...
def train_xgboost_model(df: pd.DataFrame) -> Tuple[xgb.XGBRegressor, List[str], float, Dict]:
    ...
    print(f"Debug: Model trained - MAE: Rs. {mae:.2f}, R²: {r2:.3f}, Confidence: {confidence:.1f}%")
...
def calculate_advanced_confidence(cat_data: pd.DataFrame, cv_predictions: np.ndarray, cv_actuals: np.ndarray, model_scores: List[float]) -> float:
    ...
    if len(cat_data) >= 2:
        try:
            trend_coef = np.polyfit(range(len(cat_data)), cat_data['amount'], 1)[0]
            trend_score = max(0, 100 - abs(trend_coef) / cat_data['amount'].mean() * 100)
        except:
            trend_score = 70
    else:
        trend_score = 70
    ...
    if len(cv_predictions) > 0 and len(cv_actuals) > 0:
        ...
        mean_error_pct = (errors.mean() / cv_actuals.mean() * 100) if cv_actuals.mean() > 0 else 50
...
def forecast_next_month_expenses_ensemble(df: pd.DataFrame) -> Tuple[Dict, str, float, Dict]:
    ...
    all_confidence = []
    all_metrics = []
    irregular_categories = ['accident', 'loan', 'medical emergency', 'repair', 'emergency', 'legal', 'fine', 'penalty', 'hospital', 'surgery']
    ...
    for cat in categories:
        ...
        if len(cat_data) < 4:
            ...
            print(f"  {cat}: Rs. {avg_amount:.0f} (insufficient data, using average)")
            continue
        ...
        try:
            ...
            last_2_amount = cat_data['amount'].iloc[-2] if len(cat_data) >= 2 else last_amount
            rolling_mean = cat_data['rolling_mean_3'].iloc[-1]
            
            future_X = np.array([[next_month_num, last_amount, last_2_amount, rolling_mean]])
            
            # Get ensemble prediction
            ensemble_pred, individual_preds = predict_with_ensemble(models, future_X)
            ...
            if cat.lower() not in irregular_categories:
                ...
                print(f"  {cat}: Rs. {ensemble_pred:.0f} (Confidence: {cat_confidence:.1f}%, Agreement: {cv_metrics['ensemble_agreement']:.1f}%)")
                if individual_preds:
                    pred_str = ', '.join([f"{p:.0f}" for p in individual_preds[:3]])
                    print(f"    └─ Predictions: [{pred_str}]")
            else:
                print(f"  {cat}: Rs. {ensemble_pred:.0f} (Irregular - excluded from confidence)")
...
def forecast_next_month_expenses_xgboost(df: pd.DataFrame) -> Tuple[Dict, str, float, Dict]:
    ...
    print(f"Debug: Forecasting for {next_month_label} with {len(categories)} categories")
    ...
    for cat in categories:
        ...
        print(f"Debug: '{cat}' - Predicted: Rs. {pred:.0f}, Last 3 avg: Rs. {avg_last_3:.0f}")
...
def generate_ai_insights_and_warnings(df: pd.DataFrame, forecast_data: Dict, confidence: float, metrics: Dict, next_month_label: str = "") -> Tuple[List[str], List[str]]:
    ...
    last_month_expenses = expenses[(expenses['date'] >= last_month_start) & (expenses['date'] < current_month_start)]
    two_months_ago_expenses = expenses[(expenses['date'] >= two_months_ago_start) & (expenses['date'] < last_month_start)]
    three_months_ago_expenses = expenses[(expenses['date'] >= three_months_ago_start) & (expenses['date'] < two_months_ago_start)]
    last_3_months_expenses = expenses[expenses['date'] >= three_months_ago_start]
    
    current_month_income = income[income['date'] >= current_month_start]
    last_month_income = income[(income['date'] >= last_month_start) & (income['date'] < current_month_start)]
    ...
    last_3_months_regular = last_3_months_expenses[~last_3_months_expenses['category'].str.lower().isin(irregular_categories)]
    ...
    last_3_months_avg_expense = last_3_months_regular['amount'].sum() / 3 if len(last_3_months_regular) > 0 else 0
    
    current_month_income_total = current_month_income['amount'].sum()
    last_month_income_total = last_month_income['amount'].sum()
    last_3_months_avg_income = last_3_months_income['amount'].sum() / 3 if len(last_3_months_income) > 0 else 0
    
    # Calculate forecasted total for next month (excluding irregular expenses)
    forecast_total = sum(amt for cat, amt in forecast_data.items() if cat.lower() not in irregular_categories) if forecast_data else 0
    
    # ==================== INSIGHTS ====================
    
    # 1. Next Month Forecast vs Last Month
    if last_month_expense_total > 0 and forecast_total > 0:
        month_change = ((forecast_total - last_month_expense_total) / last_month_expense_total) * 100
        
        if confidence >= 80:
            if month_change > 10:
                warnings.append(f"⚠️ Expected Increase: {next_month_label} expenses forecasted at Rs. {forecast_total:.0f} ({month_change:+.1f}% vs last month's Rs. {last_month_expense_total:.0f})")
            elif month_change < -10:
                insights.append(f"� Positive Outlook: {next_month_label} expenses expected to drop to Rs. {forecast_total:.0f} ({month_change:.1f}% vs last month)")
            else:
                insights.append(f"📊 Steady Spending: {next_month_label} expenses forecasted at Rs. {forecast_total:.0f}, similar to last month (Rs. {last_month_expense_total:.0f})")
        else:
            insights.append(f"📈 Preliminary Forecast: {next_month_label} expenses estimated at Rs. {forecast_total:.0f}. Confidence: {confidence:.0f}%")
    ...
    if last_month_expense_total > 0 and two_months_ago_expense_total > 0:
        ...
        if mom_change > 15:
            warnings.append(f"� Rising Trend: Your expenses increased by {mom_change:.1f}% last month. Last month: Rs. {last_month_expense_total:.0f}, Two months ago: Rs. {two_months_ago_expense_total:.0f}")
        elif mom_change < -15:
            insights.append(f"� Great Control: You reduced spending by {abs(mom_change):.1f}% last month compared to the previous month!")
    
    # 3. Category Analysis - Compare last month to forecast
    last_month_by_category = last_month_expenses.groupby('category')['amount'].sum().sort_values(ascending=False)
    ...
    if len(last_month_by_category) > 0 and forecast_data:
        ...
        for cat in last_month_by_category.index[:3]:  # Top 3 categories
            ...
            if forecast_cat_amt > 0 and last_month_cat_amt > 0:
                cat_change = ((forecast_cat_amt - last_month_cat_amt) / last_month_cat_amt) * 100
                
                if abs(cat_change) > 20:
                    if cat_change > 0:
                        warnings.append(f"📊 {cat}: Expected to increase by {cat_change:.0f}% (Rs. {last_month_cat_amt:.0f} → Rs. {forecast_cat_amt:.0f})")
                    else:
                        insights.append(f"� {cat}: Expected to decrease by {abs(cat_change):.0f}% (Rs. {last_month_cat_amt:.0f} → Rs. {forecast_cat_amt:.0f})")
    ...
    if last_month_income_total > 0 and last_month_expense_total > 0:
        ...
        if savings_rate < 0:
            warnings.append(f"🚨 Last Month Deficit: You spent Rs. {abs(last_month_savings):.0f} more than you earned. Income: Rs. {last_month_income_total:.0f}, Expenses: Rs. {last_month_expense_total:.0f}")
        elif savings_rate < 10:
            warnings.append(f"⚠️ Low Savings Rate: Last month you saved only {savings_rate:.1f}% (Rs. {last_month_savings:.0f}). Aim for 20%+")
        elif savings_rate >= 20 and savings_rate < 30:
            insights.append(f"✅ Healthy Savings: Last month you saved {savings_rate:.1f}% (Rs. {last_month_savings:.0f}) of your income")
        elif savings_rate >= 30:
            insights.append(f"🌟 Excellent Savings: Last month you saved {savings_rate:.1f}% (Rs. {last_month_savings:.0f})!")
    
    # 5. Forecast vs Average Comparison
    if last_3_months_avg_expense > 0 and forecast_total > 0:
        vs_avg_change = ((forecast_total - last_3_months_avg_expense) / last_3_months_avg_expense) * 100
        
        if vs_avg_change > 5:
            insights.append(f"📊 Above Average: Next month forecast (Rs. {forecast_total:.0f}) is {vs_avg_change:.1f}% above your 3-month average (Rs. {last_3_months_avg_expense:.0f})")
        elif vs_avg_change < -5:
            insights.append(f"💰 Below Average: Next month forecast (Rs. {forecast_total:.0f}) is {abs(vs_avg_change):.1f}% below your 3-month average (Rs. {last_3_months_avg_expense:.0f})")
    
    # 6. Income Stability Check
    if last_month_income_total > 0 and last_3_months_avg_income > 0:
        income_variance = abs((last_month_income_total - last_3_months_avg_income) / last_3_months_avg_income) * 100
        
        if income_variance > 20:
            insights.append(f"� Income Variance: Last month income (Rs. {last_month_income_total:.0f}) varied {income_variance:.0f}% from your 3-month average (Rs. {last_3_months_avg_income:.0f})")
    ...
    if len(last_month_by_category) > 0:
        ...
        top_pct = (top_amt / last_month_expense_total) * 100 if last_month_expense_total > 0 else 0
        
        warnings.append(f"💡 Top Category: {top_cat} was your biggest expense last month at Rs. {top_amt:.0f} ({top_pct:.0f}% of total)")
    ...
    if forecast_data:
        for cat, amt in forecast_data.items():
            if cat.lower() in irregular_categories and amt > 500:
                warnings.append(f"ℹ️ Note: {cat} (Rs. {amt:.0f}) is classified as an irregular/non-recurring expense and excluded from forecast total. Actual spending may vary.")
    ...
    if last_3_months_avg_income > 0 and forecast_total > 0:
        ...
        if forecast_to_income_ratio > 90:
            warnings.append(f"💸 High Expense Forecast: {next_month_label} expenses (Rs. {forecast_total:.0f}) will consume {forecast_to_income_ratio:.0f}% of your average income")
        elif forecast_to_income_ratio > 80:
            warnings.append(f"⚠️ Watch Your Budget: {next_month_label} expenses forecasted at {forecast_to_income_ratio:.0f}% of your income")
    
    # 9. Unusual Category Spike Detection
    if len(last_month_by_category) > 0 and len(two_months_ago_expenses) > 0:
        two_months_by_cat = two_months_ago_expenses.groupby('category')['amount'].sum()
        ...
        for cat in last_month_by_category.index[:5]:
            ...
            if two_months_cat > 0:
                spike = ((last_month_cat - two_months_cat) / two_months_cat) * 100
                
                if spike > 50 and last_month_cat > 1000:
                    warnings.append(f"� Unusual Spike: {cat} increased by {spike:.0f}% last month (Rs. {two_months_cat:.0f} → Rs. {last_month_cat:.0f})")
    
    # 10. No Recent Income Warning
    if current_month_income_total == 0 and last_month_income_total == 0:
        warnings.append("⚠️ No Recent Income: No income recorded in the last 2 months. Make sure to track all income sources")
    
    # 11. Low Confidence Warning
    if confidence < 70:
        warnings.append(f"📊 Forecast Uncertainty: Model confidence is {confidence:.0f}%. Predictions improve with more historical data")
    
    # 12. High Essential Expenses
    essential_cats = ['rent', 'utilities', 'emi', 'loan', 'insurance', 'food', 'groceries']
    essential_last_month = last_month_expenses[last_month_expenses['category'].str.lower().isin(essential_cats)]['amount'].sum()
    
    if essential_last_month > 0 and last_month_expense_total > 0:
        essential_pct = (essential_last_month / last_month_expense_total) * 100
        
        if essential_pct > 70:
            warnings.append(f"🏠 High Essential Expenses: {essential_pct:.0f}% of last month's expenses were essentials (Rs. {essential_last_month:.0f}). Limited room for savings.")
        elif essential_pct < 40:
            warnings.append(f"💡 High Discretionary Spending: Only {essential_pct:.0f}% on essentials. Review discretionary expenses to increase savings.")
    ...
    if len(last_month_expenses) > 50:  # More than 50 transactions in a month
        ...
        if len(small_transactions) > 20:  # More than 20 small transactions
            ...
            warnings.append(f"🛒 Frequent Small Purchases: {len(small_transactions)} transactions under Rs. 500 totaling Rs. {small_total:.0f}. These add up quickly!")
    ...
    if last_month_expense_total > 0 and two_months_ago_expense_total > 0 and three_months_ago_expense_total > 0:
        recent_trend = ((last_month_expense_total - two_months_ago_expense_total) / two_months_ago_expense_total) * 100
        older_trend = ((two_months_ago_expense_total - three_months_ago_expense_total) / three_months_ago_expense_total) * 100
        
        if recent_trend > 10 and older_trend > 10:
            warnings.append(f"📈 Continuous Growth: Expenses have been rising for 2+ months. Last 3 months: Rs. {three_months_ago_expense_total:.0f} → Rs. {two_months_ago_expense_total:.0f} → Rs. {last_month_expense_total:.0f}")
    
    # 15. Weekend/Lifestyle Spending Warning
    lifestyle_cats = ['entertainment', 'dining', 'travel', 'shopping', 'leisure', 'hobbies', 'gym']
    lifestyle_last_month = last_month_expenses[last_month_expenses['category'].str.lower().isin(lifestyle_cats)]['amount'].sum()
    
    if lifestyle_last_month > 0 and last_month_expense_total > 0:
        lifestyle_pct = (lifestyle_last_month / last_month_expense_total) * 100
        
        if lifestyle_pct > 40:
            warnings.append(f"🎉 High Lifestyle Spending: {lifestyle_pct:.0f}% (Rs. {lifestyle_last_month:.0f}) spent on entertainment, dining, travel, and shopping")
        elif lifestyle_pct > 30:
            warnings.append(f"🎯 Lifestyle Budget: {lifestyle_pct:.0f}% on lifestyle categories. Consider moderating for better savings")
...
async def api_insights(request: Request, body: Optional[UserEstimates] = None, userId: str = None):
    ...
    try:
        ...
        expense_forecast, next_month_label, confidence, metrics = forecast_next_month_expenses_xgboost(df)
        cleaned_forecast = {cat: amt for cat, amt in expense_forecast.items() if amt > 100}
        insights, warnings = generate_ai_insights_and_warnings(df, cleaned_forecast, confidence, metrics, next_month_label)
...
def api_forecast_get(userId: str):
    ...
    try:
        ...
        forecast_data, next_month_label, confidence, metrics = forecast_next_month_expenses_xgboost(df)
